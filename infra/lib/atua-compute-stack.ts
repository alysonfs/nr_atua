...
export class AtuaComputeStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: AtuaComputeStackProps) {
    ...
    collectorUserData.addCommands(
      ...
      '# DÉBITO TÉCNICO CONHECIDO: tokens de ServiceCredential vazios — ainda não',
      '# existe tenant/integração/credencial real provisionada (bloqueado por D7,',
      '# ver docs/decisions). O Worker sobe normalmente, mas /claim retornará 401,',
      '# tratado como erro recuperável no loop principal (Worker.cs), sem crash.',
